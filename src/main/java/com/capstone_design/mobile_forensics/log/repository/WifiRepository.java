...
@Repository
public interface WifiRepository extends JpaRepository<WifiLog, Long> {
    // 30분 전후로 범위 설정하고 50m 이내로 거리 계산하여 필터링하는 Query
    @Query(value = "SELECT * FROM wifi u WHERE " +
    ...
    List<WifiLog> findByTimestampAndLocationRange(@Param("timestamp") LocalDateTime timestamp,
